...
The requested columns must not exceed {BulkConfig.max_columns_return}. The query parameter __curves__ can be use to limit the number of columns."""
...
async def _process_request_v1(record_id: str,
    ...
    if not data_param.describe: # don't limit columns when describe parameter is True
        ...
        if nb_cols_to_return > BulkConfig.max_columns_return:
