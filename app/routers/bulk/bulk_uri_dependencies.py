...
class OsduBulkIdAccess(BulkIdAccess):
    ...
    def set_bulk_uri(record, bulk_id):
        ...
        record.data.update({'ExtensionProperties': {'wdms': {BULK_URI_FIELD: bulk_urn}}})
...
class LogBulkIdAccess(BulkIdAccess):
    ...
    @staticmethod
    def set_bulk_uri(record, bulk_id, custom_bulk_id_path: Optional[str] = None):
        LogBulkHelper.update_bulk_id(record=record, bulk_id=bulk_id,
                                     prefix=BULK_URN_PREFIX_VERSION,
                                     custom_bulk_id_path=custom_bulk_id_path)
