...
async def resolve_tenant(data_partition_id: str) -> Tenant:
    ...
    if Config.cloud_provider == 'gcp':
        return Tenant(
            ...
            project_id=Config.default_data_tenant_project_id,
            credentials=Config.default_data_tenant_credentials.value,
            bucket_name=f'{Config.default_data_tenant_project_id}-logstore-osdu'
        )

    if Config.cloud_provider == 'az':
        return Tenant(
            data_partition_id=data_partition_id,
            project_id='',
