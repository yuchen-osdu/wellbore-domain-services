...
from .test_session import create_session, with_welllog, SESSION_URL_PREFIX
...
def test_send_one_chunk_without_session(with_wdms_env, with_welllog):
    ...
    with get_client(with_wdms_env) as client:
        res = client.post(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', content=data_to_send, headers=headers)
...
def test_send_multiple_chunks_with_session_commit(with_wdms_env, with_welllog):
    ...
    with get_client(with_wdms_env) as client:
        ...
        for idx in range(0, 1000, 100):
            chunk = data.iloc[idx:idx+100].to_parquet(engine="pyarrow")
...
def test_get_data_with_ofset_filter(with_wdms_env, with_welllog):
    ...
    with get_client(with_wdms_env) as client:
        ...
        # read data
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": 0})
        pd.testing.assert_frame_equal(data, read_parquet(res.content))
        
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": "0"})
        pd.testing.assert_frame_equal(data, read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": 1})
        pd.testing.assert_frame_equal(data.tail(size-1), read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": int(size/2)})
        pd.testing.assert_frame_equal(data.tail(int(size/2)), read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": size-1})
        pd.testing.assert_frame_equal(data.tail(1), read_parquet(res.content))

        # with column filter
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": 1, "curves": "X"})
        ...
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": size})
        pd.testing.assert_frame_equal(data.tail(0), read_parquet(res.content))
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": size + 50})
        pd.testing.assert_frame_equal(data.tail(0), read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": -2})
        assert res.status_code == httpx.codes.UNPROCESSABLE_ENTITY
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"offset": "false"})
        assert res.status_code == httpx.codes.UNPROCESSABLE_ENTITY
...
def test_get_data_with_column_filter(with_wdms_env, with_welllog):
    ...
    with get_client(with_wdms_env) as client:
        ...
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"curves": "MD"})
        pd.testing.assert_frame_equal(data[['MD']], read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"curves": "X, Y, Z"})
        pd.testing.assert_frame_equal(data[['X', 'Y', 'Z']], read_parquet(res.content))

        "ignore with non existing column"
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"curves": "W, X"})
...
def test_get_data_with_limit_filter(with_wdms_env, with_welllog):
    ...
    with get_client(with_wdms_env) as client:
        ...
        # read data
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 0})
        assert res.status_code == httpx.codes.UNPROCESSABLE_ENTITY
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": -2})
        assert res.status_code == httpx.codes.UNPROCESSABLE_ENTITY

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 1})
        pd.testing.assert_frame_equal(data.head(1), read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 50})
        pd.testing.assert_frame_equal(data.head(50), read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 99})
        pd.testing.assert_frame_equal(data.head(99), read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": "3"})
        pd.testing.assert_frame_equal(data.head(3), read_parquet(res.content))

        # with column filter
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 50, "curves": "X"})
        pd.testing.assert_frame_equal(data[['X']].head(50), read_parquet(res.content))

        # if limit >= number of rows, returns all rows
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 100})
        pd.testing.assert_frame_equal(data, read_parquet(res.content))
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 150})
        pd.testing.assert_frame_equal(data, read_parquet(res.content))
...
def test_get_data_with_limit_and_offset_filter(with_wdms_env, with_welllog):
    ...
    with get_client(with_wdms_env) as client:
        ...
        # read data
        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 5, "offset": 2})
        pd.testing.assert_frame_equal(data.tail(size-2).head(5), read_parquet(res.content))

        res = client.get(f'/{WELLLOG_URL_PREFIX}/{record_id}/data', headers=headers, params={"limit": 5, "offset": size-2})
