...
@pytest.mark.parametrize("input_id, expected_code, expected_result", [
    ('123456', 200, logset(id="123456")),
    ('111111', 404, {'detail': 'Not found'}),
])
def test_get_logset(client, input_id, expected_code, expected_result):
    ...
    with when(StorageRecordServiceClientMock).get_record(...).thenAnswer(asyncreturnfn):
        response = client.get("/ddms/v2/logsets/123456")
        assert response.status_code == expected_code
        if expected_code == 200:
            assert logset(**(response.json())) == expected_result
        else:
            assert expected_result == response.json()


def test_del_logset(client):
...
def test_del_logset_recursively(client):
    with when(StorageRecordServiceClientMock).delete_record(
            id='123456',
        ...
        with when(SearchServiceClientMock).query_with_cursor(...).\
            ...
            response = client.delete("/ddms/v2/logsets/123456", headers={"recursive": "true"})
            assert response.status_code == status.HTTP_204_NO_CONTENT


def test_get_logset_versions(client):
...
def test_get_logset_at_version(client):
    ...
    with when(StorageRecordServiceClientMock).get_record_version(
        ...
        response = client.get("/ddms/v2/logsets/123456/versions/12356")
        assert response.status_code == status.HTTP_200_OK
        assert logset(**(response.json())) == expect_response


def test_set_logset(client):
    ...
    with when(StorageRecordServiceClientMock).create_or_update_records(...).thenAnswer(
        ...
        response = client.put("/ddms/v2/logsets",
