...
def client():
    ...
    with patch_async(
            'app.routers.ddms_v2.marker_ddms_v2.get_storage_record_service',
...
@pytest.mark.parametrize("input_id, expected_code, expected_result", [
    ('123456', 200, marker(acl={}, kind="opendes:wks:marker:1.0.4", legal=Legal(), id="123456")),
    ('111111', 404, {'detail': 'Not found'}),
])
def test_get_marker(client, input_id, expected_code, expected_result):
    ...
    async def asyncreturnfn(id, *args, **kwargs):
        ...
        return marker(id=id, acl={}, kind="opendes:wks:marker:1.0.4", legal=Legal())

    with when(StorageRecordServiceClientMock).get_record(...).thenAnswer(asyncreturnfn):
        response = client.get("/ddms/v2/markers/123456")
        assert response.status_code == expected_code
        if expected_code == 200:
            assert marker(**(response.json())) == expected_result
        else:
            assert expected_result == response.json()


def test_del_marker(client):
    with when(StorageRecordServiceClientMock).delete_record(
        ...
        response = client.delete("/ddms/v2/markers/123456")
        assert response.status_code == status.HTTP_204_NO_CONTENT


def test_get_marker_versions(client):
    ...
    with when(StorageRecordServiceClientMock).get_all_record_versions(
        ...
        response = client.get("/ddms/v2/markers/123456/versions")
        assert response.status_code == status.HTTP_200_OK
        assert RecordVersions(**(response.json())) == expected_response


# TODO add a negative test  -- need to forward error codes
def test_get_marker_at_version(client):
    expected_response = marker(id="123456",
                               version=12356,
                               acl={},
    ...
    with when(StorageRecordServiceClientMock).get_record_version(
        ...
        response = client.get("/ddms/v2/markers/123456/versions/12356")
        assert response.status_code == status.HTTP_200_OK
        assert marker(**(response.json())) == expected_response


def test_set_marker(client):
    ...
    with when(StorageRecordServiceClientMock).create_or_update_records(...).\
        ...
        response = client.put("/ddms/v2/markers",
                              data='[{"id": "rec1", "acl":"", "kind":"opendes:wks:marker:1.0.4", "legal":""},'
                                   ' {"id": "rec2", "acl":"", "kind":"opendes:wks:marker:1.0.4", "legal":""}]'
                              )
        assert response.status_code == status.HTTP_200_OK
