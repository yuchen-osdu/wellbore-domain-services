...
def test_welllog_number_column_should_validate_many_columns(welllog120_v3_list):
    # GIVEN a record/bulk with more than 255 columns (100_000 here)
    welllog_record = welllog120_v3_list[0].copy(deep=True)
    welllog_record.data.Curves = [Curve120(CurveID='C1', NumberOfColumns=100_000)]

    df = generate_df([f'C1[{i}]' for i in range(100_000)], [0, 1])
